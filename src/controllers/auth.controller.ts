...
export async function autenticar(_req: Request, _res: Response) {
  const body = (_req.body ?? {}) as { email?: unknown; password?: unknown }
  ...
  const credenciaisValidas =
    utilizador ? await bcrypt.compare(password, utilizador.passwordHash) : false
...
export async function renovarToken(_req: Request, _res: Response) {
  _res.status(501).json({ description: "Não implementado.", code: "NOT_IMPLEMENTED" })
  return
}

export async function terminarSessao(_req: Request, _res: Response) {
  _res.status(204).end()
  return
}

...
app.get(`${API_PREFIX}/campaigns`, async (req, res) => {
  ...
  const rows = campaigns.map((c) => {
    const beaches = Array.isArray((c as any).beaches) ? ((c as any).beaches as any[]) : []
    const beachNames = beaches.map((b) => b?.name).filter(Boolean) as string[]
...
app.get(`${API_PREFIX}/campaigns/:campaignId`, async (req, res) => {
  ...
  const beaches = Array.isArray((campaign as any).beaches)
    ? (((campaign as any).beaches as any[]) ?? [])
    : []
  const registrations = Array.isArray((campaign as any).registrations)
    ? (((campaign as any).registrations as any[]) ?? [])
    : []
  const comments = Array.isArray((campaign as any).comments)
    ? (((campaign as any).comments as any[]) ?? [])
    : []
  const wasteCollections = Array.isArray((campaign as any).wasteCollections)
    ? (((campaign as any).wasteCollections as any[]) ?? [])
    : []
  ...
  res.json({
    ...
    organizer: (campaign as any).organizer
      ? {
          id: (campaign as any).organizer.id,
          name: (campaign as any).organizer.name,
          email: (campaign as any).organizer.email
        }
      : null,
